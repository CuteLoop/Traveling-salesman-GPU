...
__device__ void order_crossover_device(const int* parent_a,
    ...
    for (int i = 0; i < n; ++i) child[i] = -1;  // no used[] init needed
...
__global__ void ga_island_kernel(int n, int generations, float mutation_rate,
    ...
    for (int generation = 0; generation < generations; ++generation) {
        ...
        // Selection sort (B3 not yet fixed)
        if (tid == 0) {
        ...
        if (tid < elite_count) {
            ...
            // B2 fix is inside order_crossover_device — no change to call site
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "VERSION: GPU-Pop B1+B2\n";
